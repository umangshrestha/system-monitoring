...
func GetMem() MemKB{
	...
	*/
	...
	for scanner.Scan(){
		...
		switch arr := strings.Fields(scanner.Text()); arr[0]{
			case "MemTotal:":  memTot, _ = strconv.ParseUint(arr[1], 10, 64)
			case "MemFree:":   memFree, _ = strconv.ParseUint(arr[1], 10, 64)
			case "Buffers:":   buffer, _ = strconv.ParseUint(arr[1], 10, 64)
			case "SwapTotal:": swapTot, _ = strconv.ParseUint(arr[1], 10, 64)
			case "SwapFree:":  swapFree, _ = strconv.ParseUint(arr[1], 10, 64)
			case "Cached:": cached, _ = strconv.ParseUint(arr[1], 10, 64)
			case "SReclaimable": sReclamaible,_ = strconv.ParseUint(arr[1], 10, 64)
			case "Shmem": shmem, _ = strconv.ParseUint(arr[1], 10, 64)
			case "Slab": slab, _ = strconv.ParseUint(arr[1], 10, 64)
		}
	}
	/*
	...
	*/
	return MemKB{
		...
		// home/umang/go/src/system-monitoring/linux/lib_memory_test.go:64: Cache: expected: 2865608, recieved: 2675428
